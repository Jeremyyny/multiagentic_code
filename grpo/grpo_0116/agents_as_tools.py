...
def load_raw_pubmedqa(path: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(raw, dict):
        for k, ex in raw.items():
            ...
            rows.append({
                ...
                "context": ex["context"],
                "ground_truth": ex["ground_truth"].strip().lower(),
            })
    ...
    for r in rows:
        if r["ground_truth"] not in ["yes", "no", "maybe"]:
            raise ValueError(f"Unexpected ground_truth={r['ground_truth']} for example_id={r['example_id']}")
    return rows
...
def parse_answer_label_lastline(text: str) -> Optional[str]:
    ...
    if not m:
        return None
    return {"YES": "yes", "NO": "no", "MAYBE": "maybe"}[m.group(1).upper()]
...
def build_manager_messages(eid: int, q: str, ctx: str) -> List[Dict[str, str]]:
    return [
...
def build_manager_sft_from_failures(
    ...
    for eid in fails:
        ...
        # step 1
        prompt1 = build_manager_messages(eid, q, ctx)
        if len(seq) == 0:
...
def train_manager_grpo_from_splits(
    ...
    def preprocess(ex: Dict[str, Any]) -> Dict[str, Any]:
        ...
        msgs = build_manager_messages(eid, ex["question"], ex["context"])
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="pqal_question_context_groundtruth.json")
    ...
    if args.stage == "make_splits":
        rows = load_raw_pubmedqa(args.data_path)
